...
if (typeof window !== 'undefined') {
  ...
  bodyEl = document.body; // Store the body element reference

  const updateWinsize = () => {
    const winSize = calcWinsize(); // Get updated window size
  ...
  // Update winsize on resize event
  window.addEventListener('resize', updateWinsize);
  window.removeEventListener('resize', updateWinsize); // If removing event listener is needed
}
...
export class Grid {
  ...
  showContent(imageCell) {
    ...
    gsap
      ...
      .add(
        () =>
          imageCell.contentItem.DOM.el.classList.add('content__item--current'),
        'showContent+=0.02'
      );
  }
